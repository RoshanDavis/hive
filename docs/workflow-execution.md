...
> **📌 Living document — current design, not a contract.** Describes the *intended* design as of **2026-05-31** (post-refactor pass, run loop extracted to `src/engine/runLoop.ts`). The code is the source of truth: **if this doc and the code disagree, trust the code and fix the doc.** Detect drift by diffing the paths under [Key files](#key-files) since the verified commit, e.g. `git log --oneline <verified>..HEAD -- src/engine/runLoop.ts src/engine/graphTraversal.ts src/contexts/runnerSession.ts`.
