...
- `src/engine/nodeData.ts` — typed accessors (`getChatMessages`, `getStorageRecords`) for the two array shapes the engine reads off `node.data`. Use them instead of `as any[]` casts.
...
**The envelope is the only output channel.** The May 2026 refactor (Phase 2) removed `lastResponse`, `outputContent`, and `upstreamEnvelope` from every built-in executor — adding any of them back is a regression. Inspectors render the response from `outputEnvelope.value`.
...
- **Color is derived, not authored.** Custom nodes (preset and script) get their `meta.color` from `getCategoryColor(category)` in `src/theme/colors.ts`. The on-disk `node.json` no longer stores a `color` field (older files with one are accepted on read and dropped on write — see `models.rs`).
...
### Styling: tokens, theme colors, shared form classes

CSS is split for theming-readiness — globals (`src/App.css` is a thin re-export) compose `src/styles/tokens.css` (CSS custom properties), `base.css` (resets), `ui-chrome.css`, `components.css`, `reactflow.css`, and `toast.css`. **Authoritative values for JS-consumed colors live in `src/theme/colors.ts`** (`ACCENT`, `DATABASE`, `NODE_COLORS`, `CATEGORY_COLORS`, `getCategoryColor`, `getStatusColor`, `EDGE`, `CANVAS`) — React Flow / MiniMap need plain strings, so these mirror the matching CSS tokens. When you change a color, update both the CSS token and the JS mirror.
